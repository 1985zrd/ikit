...
### Form.create(options) | this.$form.createForm(this, options) [#](#Form.create(options)-|-this.$form.createForm(this,-options))

使用方式如下：

#### jsx 使用方式，使用方式和 React 版 antd 一致 [#](#jsx-使用方式，使用方式和-React-版-antd-一致)
...
#### 单文件 template 使用方式 [#](#单文件-template-使用方式)
...
### validateFields/validateFieldsAndScroll [#](#validateFields/validateFieldsAndScroll)
...
#### validateFields 的 callback 参数示例 [#](#validateFields-的-callback-参数示例)
...
### Form.createFormField [#](#Form.createFormField)

用于标记 `mapPropsToFields` 返回的表单域数据，[例子](#components-form-demo-global-state)。

### this.form.getFieldDecorator(id, options) 和 v-decorator="[id, options]" [#](#this.form.getFieldDecorator(id,-options)-和-v-decorator"[id,-options]")
