<template>
  <codeBox title="栅格配置器 " description="可以简单配置几种等分栅格和间距。" :code="configureGridCode">
    <div id="components-grid-demo-playground">
      <div style="marginBottom:16px">
        <span style="marginRight:6px">Gutter (px):</span>
        <div style="width:50%">
          <ik-slider :min="0" :max="Object.keys(gutters).length - 1" v-model="gutterKey" :marks="this.gutters" :step="null" />
        </div>
        <span style="marginRight:6px">Column Count:</span>
        <div style="width:50%">
          <ik-slider :min="0" :max="Object.keys(colCounts).length - 1" v-model="colCountKey" :marks="this.colCounts" :step="null" />
        </div>
      </div>
      <ik-row :gutter="gutters[gutterKey]">
        <ik-col v-for="item in colCounts[colCountKey]" :key="item.toString()" :span="24/colCounts[colCountKey]">
          <div>Column</div>
        </ik-col>
      </ik-row>
...
<script>
import { configureGridCode } from './../codeExample'
export default {
  data () {
    const gutters = {}
