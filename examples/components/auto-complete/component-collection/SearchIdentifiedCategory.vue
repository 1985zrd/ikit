<template>
  <codeBox :title="title"
           ...
           :code="searchIdentifiedCategory">
    <div class="certain-category-search-wrapper"
         style="width: 250px">
      <ik-auto-complete class="certain-category-search"
        ...
        <template slot="dataSource">
          <ik-select-opt-group v-for="group in dataSource"
                              :key="group.title">
            <span slot="label">
              {{group.title}}
              <a style="float: right"
            ...
            <ik-select-option v-for="opt in group.children"
                             :key="opt.title"
                             :value="opt.title">
              {{opt.title}}
...
<script>
import { searchIdentifiedCategory } from '../codeExample'
const dataSource = [
  {
    title: '话题',
    children: [
...
export default {
  data () {
    return {
      title: '查询模式 - 确定类目',
      description: '查询模式 - 确定类目',
      searchIdentifiedCategory,
      dataSource
    }
  }
}
